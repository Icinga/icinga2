...
## Keep formatting in sync with ssmtp-service-notification.sh
...
## If an explicit sender was specified, set it.
(
  if [ -n "$MAILFROM" ] ; then
    /usr/bin/printf "From: %s\n" "$MAILFROM"
  fi

  /usr/bin/printf "Subject: %s\nTo: %s\n\n%b" "$SUBJECT" "$USEREMAIL" "$NOTIFICATION_MESSAGE"
) | ssmtp "$USEREMAIL"
