...
static void AssertCheckableRedundancyGroup(Checkable::Ptr checkable, int dependencyCount, int redundancyGroupCount, int memberCount)
{
	BOOST_CHECK_MESSAGE(
...
BOOST_AUTO_TEST_CASE(multi_parent)
{
	...
	parentHost1->RemoveDependency(dep3);
...
BOOST_AUTO_TEST_CASE(default_redundancy_group_registration_unregistration)
{
	...
	BOOST_CHECK_EQUAL(1, RedundancyGroup::GetRegistrySize());
	...
	BOOST_CHECK_EQUAL(0, RedundancyGroup::GetRegistrySize());
}
...
BOOST_AUTO_TEST_CASE(simple_redundancy_group_registration_unregistration)
{
	...
	BOOST_CHECK_EQUAL(2, RedundancyGroup::GetRegistrySize());
	...
	childHostD->AddDependency(depDA);
...
BOOST_AUTO_TEST_CASE(mixed_redundancy_group_registration_unregsitration)
{
	...
	// All 3 hosts share the same group, and each host has 2 members, thus 6 members in total.
	...
	childHostE->AddDependency(depEA); // Re-register depEA and instead...
