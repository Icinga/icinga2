...
{
	GetWriter()->SetManageIndexTemplate(false);
	ResumeWriter();
	
	ReceiveCheckResults(2, ServiceState::ServiceCritical);
	ReceiveCheckResults(1, ServiceState::ServiceOK);
	...
	Dictionary::Ptr host = cr->Get("host");
	BOOST_REQUIRE(host);
	BOOST_CHECK_EQUAL(host->Get("hostname"), "h1");
...
BOOST_AUTO_TEST_CASE(pause_with_pending_work)
{
	...
	REQUIRE_LOG_MESSAGE("'ElasticsearchDatastreamWriter' paused\\.", 10s);
}
