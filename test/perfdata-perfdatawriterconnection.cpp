...
public:
	TlsPerfdataWriterFixture() : PerfdataWriterTargetFixture(MakeContext())
	{
		...
		m_PdwSslContext = SetupSslContext(
			pdwCert.crtFile, pdwCert.keyFile, m_CaCrtFile.string(), "", DEFAULT_TLS_CIPHERS, DEFAULT_TLS_PROTOCOLMIN, DebugInfo()
		);
...
private:
	Shared<boost::asio::ssl::context>::Ptr MakeContext()
	{
		...
		return SetupSslContext(
			testCert.crtFile, testCert.keyFile, m_CaCrtFile.string(), "", DEFAULT_TLS_CIPHERS, DEFAULT_TLS_PROTOCOLMIN, DebugInfo()
		);
	}
