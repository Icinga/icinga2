 ...
 */
auto NewCertFromExisting(const std::shared_ptr<X509>& ca, long validFor, bool caCert = false)
{
	std::shared_ptr<EVP_PKEY> caPubKey(X509_get_pubkey(ca.get()), EVP_PKEY_free);
	return CreateCertIcingaCA(caPubKey.get(), X509_get_subject_name(ca.get()), validFor, caCert);
}
 ...
 */
std::shared_ptr<ASN1_TIME> MakeASN1TimeFrom(long seconds)
{
	auto now = time(nullptr);
	return std::shared_ptr<ASN1_TIME>(X509_time_adj_ex(nullptr, 0, seconds, &now), ASN1_TIME_free);
}
...
BOOST_FIXTURE_TEST_CASE(create_verify_ca, CertificateFixture)
{
	...
	BOOST_CHECK(X509_time_adj_ex(X509_get_notAfter(cacert.get()), 0, LEAF_VALID_FOR-1, &now));
	...
	BOOST_CHECK(X509_time_adj_ex(X509_get_notAfter(cacert.get()), 0, ROOT_VALID_FOR, &now));
...
BOOST_FIXTURE_TEST_CASE(create_verify_leaf_certs, CertificateFixture)
{
	...
	BOOST_CHECK(X509_time_adj_ex(X509_get_notAfter(cert.get()), 20, 0, &now));
	...
	PkiUtility::SignCsr(certInfo.csrFile, certInfo.crtFile, -10*24*60*60); // Expire 10 days ago!
	...
	BOOST_CHECK(X509_time_adj_ex(X509_get_notBefore(cert.get()), 0, -(now-l_2017), &now));
	BOOST_CHECK(IsCertUptodate(cert));
	BOOST_CHECK(VerifyCertificate(cacert, cert, String()));

	cacert = NewCertFromExisting(cacert, -10*24*60*60, true); // Expire the CA 10 days ago.
	BOOST_CHECK_EQUAL(1, X509_verify(cacert.get(), caprivatekey.get())); // 1 == equal, 0 == unequal, -1 == error
	BOOST_CHECK(!IsCaUptodate(cacert.get()));
	BOOST_CHECK_EXCEPTION(VerifyCertificate(cacert, cert, String()), openssl_error, assertForCertHasExpired);
	...
	auto performAssertion = [](const openssl_error& e) {
		const unsigned long* opensslCode = boost::get_error_info<errinfo_openssl_error>(e);
		BOOST_REQUIRE(opensslCode);
	...
	BOOST_CHECK_EXCEPTION(VerifyCertificate(cacert, newCACert, String()), openssl_error, performAssertion);
	BOOST_CHECK_EXCEPTION(VerifyCertificate(newCACert, cacert, String()), openssl_error, performAssertion);

	cacert = newCACert;
	PkiUtility::SignCsr(certInfo.csrFile, certInfo.crtFile); // Resign the CSR again!
	cert = GetX509Certificate(certInfo.crtFile);
	BOOST_CHECK(IsCertUptodate(cert));
	BOOST_CHECK(VerifyCertificate(cacert, cert, String()));
	...
	auto newCert = NewCertFromExisting(cert, LEAF_VALID_FOR, false);
