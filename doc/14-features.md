...
| state_check.threshold | Warning threshold values for checks.  |
...
!!! note
    ...
    When using custom resource attributes, don't separate your custom attribute keys with dots (`.`) unless you know
    what you are doing. In OpenTelemetry, dots in resource attribute keys are used to denote hierarchical relationships
    between attributes. For example, `host.os` would be interpreted as a hierarchical attribute where `host` is the
    parent and `os` is the child. Prefer using underscores to separate words in your custom attribute keys to avoid
    superfluous attribute namespacing.
...
metric point attributes. Below is an example of the full data format for both metrics and be used as a reference for
configuring your OTLP collector or backend to properly receive and process the emitted metrics.
