...
complete list of issues and PRs, please see the [milestone on GitHub](https://github.com/Icinga/icinga2/milestone/99?closed=1).
...
* Icinga DB
  ...
  * Icinga DB: Send downtime `cancel_time` only if it is cancelled. #10379
  * Icinga DB: Send only the necessary data to the `icinga:stats` Redis® stream. #10359
...
* Add `fuse.sshfs` and `fuse.*` (supported only by Monitoring Plugins) to the list of default disk exclude types. #9649
...
* CMake: Raise the minimum required policy to `3.8`. #10402
