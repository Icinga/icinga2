...
std::vector<DependencyGroup::Ptr> Checkable::GetDependencyGroups() const
{
	...
	for (auto [_, dependencyGroup] : m_DependencyGroups) {
...
void Checkable::AddDependency(const Dependency::Ptr& dependency)
{
	...
	DependencyGroup::Ptr dependencyGroup(new DependencyGroup(dependency->GetRedundancyGroup(), dependencies));
	m_DependencyGroups.emplace(dependencyGroupKey, DependencyGroup::Register(dependencyGroup));
	...
	if (existingGroup) {
		...
		DependencyGroup::OnChildRemoved(existingGroup, {dependencies.begin(), dependencies.end()});
	}
...
void Checkable::RemoveDependency(const Dependency::Ptr& dependency, bool runtimeRemoved)
{
	...
	std::set<Dependency::Ptr> dependencies(DependencyGroup::Unregister(existingGroup, this));
	...
	if (!dependencies.empty()) {
		newDependencyGroup = new DependencyGroup(dependency->GetRedundancyGroup(), dependencies);
		m_DependencyGroups.emplace(dependencyGroupKey, DependencyGroup::Register(newDependencyGroup));
	}
