...
DependencyGroup::Ptr DependencyGroup::Register(const DependencyGroup::Ptr& dependencyGroup)
{
	...
	if (auto [it, inserted] = m_Registry.insert(dependencyGroup); !inserted) {
		dependencyGroup->MoveDependenciesTo(*it);
		return *it;
	}
...
std::set<Dependency::Ptr> DependencyGroup::Unregister(const DependencyGroup::Ptr& dependencyGroup, const Checkable::Ptr& child)
{
	...
	if (auto it(m_Registry.find(dependencyGroup)); it != m_Registry.end()) {
		...
		if (existingGroup->GetDependenciesCount() == dependencies.size()) {
			m_Registry.erase(it);
		}
...
/**
 * Check whether the dependency group has a parent Checkable referenced by the same config as the provided dependency.
 *
 * Same config means any dependency object that produces the exact same key (CompositeKeyType) as the provided one.
 *
 * @param dependency The dependency to look the parent Checkable for.
 *
 * @return bool
 */
...
/**
 * Move the dependency objects of the current dependency group to the provided dependency group (destination).
 *
 * @param dest The dependency group to move the dependencies to.
 */
void DependencyGroup::MoveDependenciesTo(const DependencyGroup::Ptr& dest)
{
