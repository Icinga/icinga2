...
void InfluxdbCommonWriter::CheckResultHandler(const Checkable::Ptr& checkable, const CheckResult::Ptr& cr)
{
	...
	Dictionary::Ptr metadataFields;
	if (GetEnableSendMetadata()) {
		metadataFields = new Dictionary();

		if (service)
			metadataFields->Set("state", new InfluxdbInteger(service->GetState()));
		else
			metadataFields->Set("state", new InfluxdbInteger(host->GetState()));

		metadataFields->Set("current_attempt", new InfluxdbInteger(checkable->GetCheckAttempt()));
		metadataFields->Set("max_check_attempts", new InfluxdbInteger(checkable->GetMaxCheckAttempts()));
		metadataFields->Set("state_type", new InfluxdbInteger(checkable->GetStateType()));
		metadataFields->Set("reachable", checkable->IsReachable());
		metadataFields->Set("downtime_depth", new InfluxdbInteger(checkable->GetDowntimeDepth()));
		metadataFields->Set("acknowledgement", new InfluxdbInteger(checkable->GetAcknowledgement()));
		metadataFields->Set("latency", cr->CalculateLatency());
		metadataFields->Set("execution_time", cr->CalculateExecutionTime());
	}

	m_WorkQueue.Enqueue([this, checkable, cr, tmpl = std::move(tmpl), metadataFields = std::move(metadataFields)]() {
