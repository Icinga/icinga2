...
void ElasticsearchDatastreamWriter::ManageIndexTemplate()
{
	...
	while (true) {
		...
		try {
			Dictionary::Ptr jsonResponse = TrySend(indexTemplateUrl, std::move(templateJson));
			Log(LogInformation, "ElasticsearchDatastreamWriter")
...
void ElasticsearchDatastreamWriter::CheckResultHandler(const Checkable::Ptr& checkable, const CheckResult::Ptr& cr)
{
	...
	m_WorkQueue.Enqueue([this, host = std::move(host), service = std::move(service), ecsHost = std::move(ecsHost),
	    ecsService = std::move(ecsService), cr = std::move(cr), checkable = std::move(checkable),
		checkResult = std::move(checkResult)]()
	{
		...
		String datastreamName = "metrics-icinga2." + checkable->GetCheckCommandRaw() + "-" + datastreamNamespace;
