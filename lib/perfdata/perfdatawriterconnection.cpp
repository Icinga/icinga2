...
PerfdataWriterConnection::PerfdataWriterConnection(
	...
	: m_VerifySecure(verifySecure), m_SslContext(std::move(sslContext)), m_Name(std::move(name)),
	  m_Host(std::move(host)), m_Port(std::move(port)), m_DisconnectTimer(IoEngine::Get().GetIoContext()),
		...
		while (true) {
			try {
				...
				Log(LogCritical, "PerfdataWriterConnection")
					<< "Error while sending to '" << m_Host << ":" << m_Port << "': " << ex.what();
	...
	IoEngine::SpawnCoroutine(m_Strand, [&, keepAlive = PerfdataWriterConnection::Ptr(this)](boost::asio::yield_context yc) {
		while (true) {
			try {
				...
				Log(LogCritical, "PerfdataWriterConnection")
					<< "Error while sending to '" << m_Host << ":" << m_Port << "' for '" << m_Name << "': " << ex.what();
...
OutgoingHttpRequest PerfdataWriterConnection::PrepareRequest()
{
	std::promise<OutgoingHttpRequest> promise;

	IoEngine::SpawnCoroutine(m_Strand, [&, keepAlive = PerfdataWriterConnection::Ptr(this)](boost::asio::yield_context yc) {
		try {
			EnsureConnected(std::move(yc));

			promise.set_value(OutgoingHttpRequest{m_Stream});
...
{
	std::promise<bool> p;
	boost::asio::post(m_Strand, [&]() { p.set_value(m_State != State::initial); });
