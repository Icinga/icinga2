...
void PerfdataWriterConnection::Send(boost::asio::const_buffer data)
{
	...
	IoEngine::SpawnCoroutine(
		...
		Log(LogDebug, "PerfdataWriterConnection") << "Send()::1";
		try {
			EnsureConnected(yc);
			Log(LogDebug, "PerfdataWriterConnection") << "Send()::2";

			std::visit([&](auto& stream) {
				boost::asio::async_write(*stream, data, yc);
				stream->async_flush(yc);
			}, m_Stream);
			Log(LogDebug, "PerfdataWriterConnection") << "Send()::3";

			promise.set_value();
		} catch (const std::exception&) {
			Log(LogDebug, "PerfdataWriterConnection") << "Send()::E";
...
HttpResponse PerfdataWriterConnection::Send(HttpRequest& request)
{
	...
	IoEngine::SpawnCoroutine(
		...
		try {
			...
			std::visit([&](auto& stream) {
				boost::beast::http::async_read(*stream, m_Streambuf, parser, yc);
			}, m_Stream);
	...
	while (m_State != State::connected) {
		if (m_Stopped) {
			const boost::system::error_code ec{
				boost::asio::error::operation_aborted, boost::system::system_category()
			};
