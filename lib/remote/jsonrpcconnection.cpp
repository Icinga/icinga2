...
void JsonRpcConnection::HandleIncomingMessages(boost::asio::yield_context yc)
{
	...
	AtomicDuration::Clock::time_point readStarted, readFinished, processingStarted;
	...
	while (!m_ShuttingDown) {
		...
		if (m_Endpoint) {
			readFinished = AtomicDuration::Clock::now();
		}
		...
		try {
			...
			if (m_Endpoint) {
				m_Endpoint->AddInputTimes(readFinished - readStarted, cpuBoundDuration, AtomicDuration::Clock::now() - processingStarted);
			}
