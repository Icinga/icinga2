 ...
 * with each attempt, up to a maximum cap (30s). This helps to avoid overwhelming the OpenTelemetry collector
				...
				std::visit(
					[this, &yc](auto& stream) {
						...
						if constexpr (std::is_same_v<std::decay_t<decltype(stream)>, Shared<AsioTlsStream>::Ptr>) {
							...
							if (m_ConnInfo.SecureVerify && !tlsStream.IsVerifyOK()) {
								BOOST_THROW_EXCEPTION(std::runtime_error(
									"TLS certificate validation failed: " + tlsStream.GetVerifyError()
								));
							}
						}
					},
	...
	if (auto ct = responseMsg[http::field::content_type]; ct != "application/x-protobuf") {
		...
		if (auto throttle = responseMsg[http::field::retry_after]; !throttle.empty()) {
			try {
				...
				Log(LogWarning, "OTelExporter")
					<< "Failed to parse 'Retry-After' header from OpenTelemetry collector response: " << ex.what();
			}
		}
...
void OTel::SetAttribute(Attribute& attr, Key&& key, AttrVal&& value)
{
	if constexpr (std::is_same_v<std::decay_t<Key>, std::string_view>) {
		...
		// Only recent Protobuf versions provide an overload for std::string_view, so we need
		// to convert it to std::string explicitly to avoid compilation errors.
		attr.set_key(std::string(key));
	} else {
		static_assert(std::is_same_v<std::decay_t<Key>, String>, "Attribute key must be convertible to String.");
		if (key.IsEmpty()) {
			BOOST_THROW_EXCEPTION(std::invalid_argument("OTel attribute key must not be empty."));
		}
		attr.set_key(std::forward<Key>(key));
	}

	if constexpr (std::is_same_v<std::decay_t<AttrVal>, std::string_view>) {
		attr.mutable_value()->set_string_value(std::string(value));
	} else if constexpr (std::is_same_v<std::decay_t<AttrVal>, String>) {
		attr.mutable_value()->set_string_value(std::forward<AttrVal>(value));
	} else {
		static_assert(std::is_same_v<std::decay_t<AttrVal>, Value>, "Attribute value must be of type Value.");
		...
		switch (value.GetType()) {
			...
			case ValueNumber:
				...
				break;
			case ValueString:
				attr.mutable_value()->set_string_value(std::move(value.template Get<String>()));
				break;
			default:
				BOOST_THROW_EXCEPTION(std::invalid_argument(
