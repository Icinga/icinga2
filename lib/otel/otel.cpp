...
void OTel::Start()
{
	...
	IoEngine::SpawnCoroutine(m_Strand, [this, keepAlive](boost::asio::yield_context yc) { Connect(yc); });
 ...
 * connection is established. Once a connection has successfully been established, it sets the @c m_ConnReady
 * flag to signal its waiters. When the connection is lost while exporting metrics, the @c m_ConnReady flag can
 * be cleared to signal this method to attempt reconnection.
 *
...
{
	Defer cleanup{[this] { m_ConnReady.Set(); }};
	...
	while (!m_Stopped) {
		...
		// Wait until someone clears m_ConnReady (e.g., on connection loss) to wake us up and reconnect.
		m_ConnReady.WaitForClear(yc);
	}
}
...
void OTel::ExportLoop(boost::asio::yield_context& yc)
{
	...
	while (!m_Stopped) {
		m_ConnReady.WaitForSet(yc);
		m_Export.WaitForSet(yc);

		for (int attempt = 1; m_Stream; ++attempt) {
			try {
				Export(yc);
				m_Request.reset();
				m_Export.Clear();
				...
				m_Stream.reset();
				m_ConnReady.Clear(); // Force reconnect on next export attempt.
			}
		}
	}
}
...
void OTel::Export(boost::asio::yield_context& yc) const
{
	...
	responseMsg.ParseHeader(buf, yc);
	responseMsg.ParseBody(buf, yc);
