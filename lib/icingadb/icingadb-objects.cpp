...
)
{
	...
	for (auto& dependencyGroup : checkable->GetDependencyGroups()) {
		...
		if (!dependencyGroup->IsRedundancyGroup()) {
			...
			// out whether we have to sync the shared edge state.
			...
			if (runtimeUpdates || m_DumpedGlobals.DependencyGroup.IsNew(redundancyGroupId)) {
				...
				// Serialize and sync the redundancy group state information a) to the RedundancyGroupState and
				// b) to the DependencyEdgeState HMSETs. The latter is shared by all child Checkables of the current
