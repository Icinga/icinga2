...
/**
 * Generate a unique identifier for the provided redundancy group.
 *
 * The identifier is generated by summing up the names of all the parent dependencies of the specified members.
 *
 * @param redundancyGroup The name of the redundancy groups.
 * @param members All the dependencies that are part of the provided redundancy group.
 *
 * @return String - Returns a unique identifier for the redundancy group.
 */
String IcingaDB::GetRedundancyGroupIdentifier(const String& redundancyGroup, const std::set<Dependency::Ptr>& members)
{
	if (Checkable::IsDefaultRedundancyGroup(redundancyGroup)) {
		return "";
	}

	auto redundancyGroupName(redundancyGroup);
	for (auto& dependency : members) {
		// Sum up all the parent names to generate a unique id for the redundancy group down below.
		redundancyGroupName += "!" + dependency->GetParent()->GetName();
	}

	return HashValue(new Array({m_EnvironmentId, redundancyGroupName}));
}
