...
private:
	...
	void InsertCheckableDependencies(const Checkable::Ptr& checkable, std::map<String, std::vector<String>>& hMSets,
	...
	void SendRedundancyGroupMemberRemoved(const Shared<RedundancyGroup>::Ptr& redundancyGroup, const Dependency::Ptr& removedMember);
	...
	static Dictionary::Ptr SerializeDependencyState(const Dependency::Ptr& dep, RedundancyGroup::State state);
	static Dictionary::Ptr SerializeRedundancyGroup(const Shared<RedundancyGroup>::Ptr& redundancyGroup, bool serializeState = false);
	...
	static void RedundancyGroupMemberRemovedHandler(const Shared<RedundancyGroup>::Ptr& redundancyGroup, const Dependency::Ptr& dep);
