...
private:
	 ...
	 * It is not used for any history or heartbeat queries, which are sent using @c m_HistoryCon.
	 ...
	 * It is not used for any other type of queries, ensuring that history and heartbeat operations
	 * do not interfere with other Redis operations. This connection is also the leader for all other
	 * Redis connections including @c m_Rcon, and is the only source of truth for all IcingaDB Redis
