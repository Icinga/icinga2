...
/**
 * Constructor for PendingDependencyItem.
 *
 * This constructor initializes a @c PendingDependencyItem with the specified dependency group, child checkable,
 * state flag, and dirty bits. The ID field in the base @c PendingQueueItem struct is constructed as a pair of
 * the child (or nullptr if `isState` is true) and the dependency group to ensure uniqueness in the container.
 *
 * @param depGroup The dependency group associated with this pending item.
 * @param child The child checkable associated with this pending item.
 * @param isState A flag indicating whether this item represents a dependency group state update.
 * @param bits The dirty bits indicating the type of updates required for this item.
 */
PendingDependencyItem::PendingDependencyItem(const DependencyGroup::Ptr& depGroup, const Checkable::Ptr& child, bool isState, uint32_t bits)
	: PendingQueueItem{std::make_pair(isState ? nullptr : child, depGroup), bits}, DepGroup{depGroup}, Child{child}
{
}
...
void IcingaDB::EnqueueDependencyChildRegistered(const DependencyGroup::Ptr& depGroup, const Checkable::Ptr& child)
{
	if (GetActive() && m_Rcon && m_Rcon->IsConnected()) {
		{
			std::lock_guard lock(m_PendingItemsMutex);
			m_PendingItems.insert(PendingDependencyItem{depGroup, child, false, DependencyChildRegistered});
		}
		m_PendingItemsCV.notify_one();
	}
}
...
)
{
...

	{
		...
		if (auto it(m_PendingItems.find(std::make_pair(child, depGroup))); it != m_PendingItems.end()) {
			...
			if (auto sit = m_PendingItems.find(std::make_pair(nullptr, depGroup)); sit != m_PendingItems.end()) {
				if (removeGroup) {
					...
					m_PendingItems.modify(sit, [&child, &depGroup](PendingItemVariant& item) {
						if (auto& pdit = std::get<PendingDependencyItem>(item); pdit.Child == child) {
							...
							pdit.Child = depGroup->GetAnyChild();
						}
					});
				}
			}
...
void IcingaDB::EnqueueDependencyGroupStateUpdate(const DependencyGroup::Ptr& depGroup, const Checkable::Ptr& child)
{
	if (GetActive() && m_Rcon && m_Rcon->IsConnected()) {
		{
			std::lock_guard lock(m_PendingItemsMutex);
			m_PendingItems.insert(PendingDependencyItem{depGroup, child, true, DependencyGroupState});
		}
		m_PendingItemsCV.notify_one();
	}
}
