...
std::chrono::duration<double> IcingaDB::DequeueAndProcessOne(std::unique_lock<std::mutex>& lock)
{
	...
	for (auto it(seqView.begin()); it != seqView.end(); ++it) {
		...
		if (auto ptr = std::get_if<queue::PendingConfigItem>(&it->Item); ptr) {
			cobj = ptr->Object;
		} else if (auto ptr = std::get_if<queue::PendingDependencyEdgeItem>(&it->Item)) {
			cobj = ptr->Child;
		}
 ...
 * @param icingadb The IcingaDB instance to use for executing Redis queries.
 */
void IcingaDB::ProcessQueueItem(const icingadb::task_queue::PendingDependencyGroupStateItem& item)
{
...
)
{
	...
	{
		std::lock_guard lock(m_PendingItemsMutex);
		if (m_PendingItems.erase(queue::PendingQueueItem{queue::PendingDependencyEdgeItem{depGroup, child}}) > 0) {
			...
			if (removeGroup) {
				...
				m_PendingItems.erase(queue::PendingQueueItem{queue::PendingDependencyGroupStateItem{depGroup}});
			}
		}
	}
