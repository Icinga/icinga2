...
PendingQueueItem::PendingQueueItem(std::string id, int dirtyBits)
	: DirtyBits{dirtyBits & DirtyBitsAll}, ID{std::move(id)}, EnqueueTime{std::chrono::steady_clock::now()}
{
}

PendingConfigItem::PendingConfigItem(const ConfigObject::Ptr& obj, int bits)
	: PendingQueueItem{MakeUniqueQueueID(obj), bits}, Object{obj}
{
}

PendingDependencyItem::PendingDependencyItem(const DependencyGroup::Ptr& depGroup, const Checkable::Ptr& child, int bits)
	: PendingQueueItem{MakeUniqueQueueID(child, depGroup), bits}, DepGroup{depGroup}, Child{child}
{
}

PendingDependencyItem::PendingDependencyItem(const DependencyGroup::Ptr& depGroup, const Checkable::Ptr& child, const String& id, int bits)
...
void IcingaDB::PendingItemsThreadProc()
{
	...
	// For easier access to the sequenced index of the pending items container.
	using SequencedView = boost::multi_index::nth_index<PendingItemsSet, 1>::type;
 ...
 *
 * @param citem The pending item containing the object and its dirty bits.
 */
void IcingaDB::ProcessPendingItem(const PendingConfigItem& citem)
{
	if (citem.DirtyBits & ConfigDelete) {
		String typeName = GetLowerCaseTypeNameDB(citem.Object);
		m_Rcon->FireAndForgetQueries(
			{
				{"HDEL", m_PrefixConfigObject + typeName, GetObjectIdentifier(citem.Object)},
				{"HDEL", m_PrefixConfigCheckSum + typeName, GetObjectIdentifier(citem.Object)},
				{
					"XADD",
					"icinga:runtime",
					"MAXLEN",
					...
					"id",
					GetObjectIdentifier(citem.Object),
					"runtime_type",
					"delete"
				}
			}
	...
	if (citem.DirtyBits & ConfigUpdate) {
		std::map<String, std::vector<String>> hMSets;
		std::vector<Dictionary::Ptr> runtimeUpdates;
		CreateConfigUpdate(citem.Object, GetLowerCaseTypeNameDB(citem.Object), hMSets, runtimeUpdates, true);
		ExecuteRedisTransaction(m_Rcon, hMSets, runtimeUpdates);
	}

	if (auto checkable = dynamic_pointer_cast<Checkable>(citem.Object); checkable) {
		if (citem.DirtyBits & FullState) {
			UpdateState(checkable, citem.DirtyBits);
		}
		if (citem.DirtyBits & NextUpdate) {
 ...
 */
void IcingaDB::EnqueueConfigObject(const ConfigObject::Ptr& object, int bits)
{
...
{
	if (dependencies.empty() || !m_Rcon || !m_Rcon->IsConnected()) {
	...
	if (!child->HasAnyDependencies()) {
		...
		// This might be a no-op in some cases (e.g. if the childs only dependency was the one that we just canceled
