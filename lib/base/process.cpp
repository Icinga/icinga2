...
extern void ManagerReadLoop(uintptr_t mgr);
extern void ManagerWriteLoop(uintptr_t mgr);
...
void Process::InitializeSpawnHelper()
{
	if (!l_ProcMgr) {
		...
		if (pid == 0) {
			...
			std::thread readLoop (&ManagerReadLoop, l_ProcMgr->Mgr);
			std::thread writeLoop (&ManagerWriteLoop, l_ProcMgr->Mgr);
		...
		std::thread(&ManagerReadLoop, l_ProcMgr->Mgr).detach();
		std::thread(&ManagerWriteLoop, l_ProcMgr->Mgr).detach();
	}
}
