		...
		if (!forced) {
			...
			if (tp) {
				...
				if (!tp->IsInside(ts)) {
					...
					Log msg (LogNotice, "CheckerComponent");

					msg << "Skipping check for object '" << checkable->GetName()
						<< "', as not in check period '" << tp->GetName() << "'";

					if (nextCheck > 0) {
						nextCheck += 0.01;
						msg << ", until " << Utility::FormatDateTime("%Y-%m-%d %H:%M:%S %z", nextCheck);
					}
